...
class ModeBasic(tk.Frame):  
    ...
    def display(self,stagenum):
        ...
        if self.stage == 1 :
            ...
            img = np.zeros((1928, 1088, 4), np.uint8)
            img[:, :, 3] = 255

            cv2.line(img, (100, 100), (400, 400), (0, 0, 255, 255), thickness=5)

            # Convert the OpenCV image to a PIL image
            pil_img = Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGRA2RGBA))

            # Convert the PIL image to a Tkinter-compatible image
            tk_img = PhotoImage(image=pil_img)

            # Merge the OpenCV image with the background image
            merged_img = Image.alpha_composite(self.stage1, pil_img)

            # Convert the merged image to a Tkinter-compatible image
            merged_tkimg = PhotoImage(merged_img)

            stage1 = Label(self, image=merged_tkimg)
