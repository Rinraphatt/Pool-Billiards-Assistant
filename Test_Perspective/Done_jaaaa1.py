...
while True:
    ...
    if mainStage == 'multiplayerMode':
        ...
        if "Black" not in updatedBall and  check_black is None:
            check_black = time.time()
        elif "Black" not in updatedBall and  check_black is not None:
    ...
    if whitePos != -1 and len(updatedBall) != 0:
        if isP1:
            ...
            if prevBallPos != updatedBall:
                acurency_p2 += 1
        center = (int(avg_white[0]),int(avg_white[1]))
        # Draw on black frame
        cv2.circle(mac, center, 30, (255, 255, 255), 5, cv2.LINE_AA)
        cv2.circle(mac, center, 200, (255, 255, 255), 6, cv2.LINE_AA)
        #avg_center_x,avg_center_y = avg_white
        x_o,y_o = realPosition(center[0],center[1])
        print(center)
        print(x_o,y_o)
        mask = np.zeros_like(table_frame)
        cv2.circle(mask, (x_o,y_o), 200, (255, 255, 255), -1, cv2.LINE_AA)
        masked_img = cv2.bitwise_and(table_frame, mask)
        ...
        if contours:
            ...
            height_cue, width_cue, _ = mac.shape
            if start_x < 0:
                start_x = 0
            ...
            if abs(start_x - ax_start) < 100 and abs(start_y - ay_start) < 100 and abs(end_x - ax_end) < 100 and abs(end_y - ay_end) < 100 :
                if len(list_start) < 5 :
                    ...
                    cv2.line(mac, (round(ax_start), round(ay_start)), (round(ax_end), round(ay_end)), (255, 255, 255), 10)
                    isDraw = True
            else:
                list_start.clear()
                list_end.clear()
                ...
                if len(lines) >= 2:
                    ...
                    if (ver == 'up' and hor == 'left'):
                        ...
                        if (y_top == output_min and (y_bottom == output_height or x_bottom == output_width)):
                            showLine(mac,(x_top, y_top),(x_top-abs(x_bottom-x_top), output_height))
                            x4, y4 = (x_top-abs(x_bottom-x_top), output_height)

                        elif (x_top == output_min and (x_bottom == output_width or y_bottom == output_height)):
                            showLine(mac,(x_top, y_top), (output_width,y_top-abs(y_top-y_bottom)))
                    ...
                    if (ver == 'up' and hor == 'right'):
                        ...
                        if (y_top == output_min and (y_bottom == output_height or x_bottom == output_min)):
                            ...
                            showLine(mac,(x_top, y_top), (output_min,y_top-abs(y_top-y_bottom)))
                    ...
                    if (ver == 'down' and hor == 'left'):
                        ...
                        if ((y_top == output_min or x_top == output_width) and y_bottom == output_height):
                            showLine(mac,(x_bottom, y_bottom), (x_bottom-abs(x_bottom-x_top), 0))
                            x4, y4 = (x_bottom-abs(x_bottom-x_top), output_min)

                        elif ((x_top == output_width or y_top == output_min) and x_bottom == output_min):
                            showLine(mac,(x_bottom, y_bottom), (output_width,y_bottom+abs(y_top-y_bottom)))
                    ...
                    if (ver == 'down' and hor == 'right'):
                        ...
                        if ((y_top == output_min or x_top == output_min) and y_bottom == output_height):
                            ...
                            showLine(mac,(x_bottom, y_bottom), (output_min,y_bottom+abs(y_top-y_bottom)))
                    ...
                    cv2.putText(mac, "Ver : " + ver + ' Hor : ' + hor , (100, 300),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
                    # Create Third reflex line
                    if (ver == 'up' and hor == 'left'):
                        if (y3 == output_height and y4 == output_min):
                            if x4 < output_min:
                                showLine(mac,(output_min, int(findSlope(x3, y3, x4, y4, "y", output_min)[2])), (abs(x4), 0))

                            else:
                                showLine(mac,(x4, y4), (x4-abs(x4-x3), output_height))    

                        elif (x3 == output_width and x4 == output_min):
                            if y4 < output_min:
                                showLine(mac,(int(findSlope(x3, y3, x4, y4, "x", output_min)[2]), output_min), (0, abs(y4)))

                            else:
                                showLine(mac,(x4, y4), (output_width, y4-abs(y4-y3)))

                    if (ver == 'up' and hor == 'right'):
                        if (y3 == output_height and y4 == output_min):
                            
                            if x4 > output_width:
                                showLine(mac,(output_width, int(findSlope(x3, y3, x4, y4, "y", output_width)[2])), (output_width-abs(x4-output_width), output_min))

                            else:
                                showLine(mac,(x4, y4), (x4+abs(x3-x4), output_height))

                        elif (x3 == output_min and x4 == output_width):
                            if y4 < output_min:
                                showLine(mac,(int(findSlope(x3, y3, x4, y4, "x", output_min)[2]), output_min), (output_width, abs(y4)))

                            else:
                                showLine(mac,(x4, y4), (output_min,y4-abs(y4-y3)))

                    if (ver == 'down' and hor == 'left'):
                        if (y3 == output_min and y4 == output_height):
                            if x4 < output_min:
                                showLine(mac,(output_min, int(findSlope(x3, y3, x4, y4, "y", output_min)[2])), (abs(x4), output_height))

                            else:
                                showLine(mac,(x4, y4), (x4-abs(x4-x3), output_min))

                        elif (x3 == output_width and x4 == output_min):
                            if y4 > output_height:
                                showLine(mac,(int(findSlope(x3, y3, x4, y4, "x", output_height)[2]), output_height), (output_min, output_height-(y4-output_height)))
                            else:
                                showLine(mac,(x4, y4), (output_width, y4-abs(y4-y3)))
                    ...
                    if (ver == 'down' and hor == 'right'):
                        if (y3 == output_min and y4 == output_height):
                            if x4 > output_width:
                                showLine(mac,(output_width, int(findSlope(x3, y3, x4, y4, "y", output_width)[2])), (output_width-abs(output_width-x4), output_height))
                            ...
                            if y4 > output_height:
                                showLine(mac,(int(findSlope(x3, y3, x4, y4, "x", output_height)[2]), output_height), (output_width, output_height-abs(y4-output_height)))
