...
def get_reference_image(img_resolution=(1920, 1080)):
    ...
    img = np.ones((height, width, 1), np.uint8) * 0
    return img
...
def get_destination_array(rect):
    ...
    """
    (tl, bl, tr, br) = rect  # Unpack the values
...
def get_perspective_transform(stream, screen_resolution, prop_file=None):
    """
...
while True:
    ...
    #show_full_frame(frame)
    cv2.imshow("Test",get_reference_image())
    print(get_perspective_transform(cap,(1920,1080)))
